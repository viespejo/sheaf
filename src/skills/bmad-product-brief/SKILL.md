...
Check activation context immediately from `{arguments}`:
...
- Acknowledge what you received — but **DO NOT read document files yet**. Note their paths for Stage 2's contextual discovery scans. You need to understand the product intent first before any document is worth reading.
